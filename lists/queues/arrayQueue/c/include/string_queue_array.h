 ...
 *
 * Description:
 * Implementation of a string type queue using an array list data structure.  
...
struct array_queue {
    ...
    char **queue;         // Array to hold the queue
};
...
int enqueue(char*, struct array_queue*);
int dequeue(struct array_queue*, char*);
